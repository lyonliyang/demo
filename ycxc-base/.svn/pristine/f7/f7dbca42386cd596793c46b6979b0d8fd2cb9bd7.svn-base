...
public class AlipaySubmit {
	 ...
	 */
	private static final String ALIPAY_GATEWAY_NEW = AlipayConfig.ALIPAY_PAY_GATEWAY;
	 ...
	 * @return 提交表单HTML文本
	 */
	public static String buildRequest(final Map<String, String> sParaTemp,
			final String strMethod, final String strButtonName,
			final String signType) {
		// 待请求参数数组
		final Map<String, String> sPara = buildRequestPara(sParaTemp, signType);
	 ...
	 * @throws Exception
	 */
	public static String buildRequest(final String strParaFileName,
			final String strFilePath, final Map<String, String> sParaTemp,
			final String signType)
			throws Exception {
		// 待请求参数数组
		final Map<String, String> sPara = buildRequestPara(sParaTemp, signType);

		final HttpProtocolHandler httpProtocolHandler = HttpProtocolHandler
				.getInstance();
	 ...
	 * @return 签名结果字符串
	 */
	public static String buildRequestMysign(final Map<String, String> sPara,
			final String signType) {
		final String prestr = AlipayCore.createLinkString(sPara); // 把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
		String mysign = "";
	...
	public static Map<String, String> buildRequestPara(
			final Map<String, String> sParaTemp, final String signType) {
		// 除去数组中的空值和签名参数
		final Map<String, String> sPara = AlipayCore.paraFilter(sParaTemp);
		// 生成签名结果
		final String mysign = buildRequestMysign(sPara, signType);
	...
	private static NameValuePair[] generatNameValuePair(
		...
		final NameValuePair[] nameValuePair = new NameValuePair[properties
				.size()];
		int i = 0;
	...
	public static String query_timestamp() throws MalformedURLException,
		...
		final String strUrl = ALIPAY_GATEWAY_NEW
		...
		final Document doc = reader.read(new URL(strUrl).openStream());

		final List<Node> nodeList = doc.selectNodes("//alipay/*");
		...
		for (final Node node : nodeList) {
			...
			if (node.getName().equals("is_success")
				...
				// 判断是否有成功标示
				final List<Node> nodeList1 = doc
						.selectNodes("//response/timestamp/*");
				for (final Node node1 : nodeList1) {
